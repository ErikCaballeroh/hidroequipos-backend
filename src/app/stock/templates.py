...
def generate_bulk_restock_email(supplier_name: str, items: list[dict]) -> str:
    items_html = ""
    for item in items:
        items_html += f"""
        <div class="data-row">
            <span class="data-label" style="flex: 2;">{item['product_name']} <br><small>({item['product_code']})</small></span>
            <span class="data-value" style="flex: 1;">{item['quantity']} unidades</span>
        </div>
        """
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Solicitud de Reabastecimiento Múltiple — Hidroequipos</title>
    <style>
        @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap');
...
<body>
    <div class="container">
        ...
        <div class="body">
            <p>Hola <strong>{supplier_name}</strong>,</p>
            <p>Por medio de la presente, solicitamos amablemente el reabastecimiento de los siguientes productos para nuestro inventario. A continuación los detalles de la solicitud:</p>
